...
void update(const sf::Vector2f &mousePosition, Eye &leftEye, Eye &rightEye)
{
    ...
    if (leftDelta.x > 35)
    {
        leftDelta.x = 35 * cos(abs(leftDelta.y) / 70);
    }
    else if (leftDelta.x < -35)
    {
        leftDelta.x = -35 * cos(abs(leftDelta.y) / 70);
    }
    if (leftDelta.y > 70)
    ...
    leftEye.pupil.setPosition({leftEyePos.x + 35 / (float(WINDOW_WIDTH) / 2) * leftDelta.x + 35,
    ...
    rightEye.pupil.setPosition({rightEyePos.x + 35 / (float(WINDOW_WIDTH) / 2) * rightDelta.x + 35,
